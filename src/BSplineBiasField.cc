...
BSplineBiasField::BSplineBiasField()
{
	...
	//for (i = 0; i < BSpline<TReal>::LookupTableSize; i++) {
...
double BSplineBiasField::Approximate(double *x1, double *y1, double *z1, double *bias, int no)
{
	...
	// Allocate memory
	//double ***dx = ::Allocate<double>(_x, _y, _z);
	//double ***ds = ::Allocate<double>(_x, _y, _z);
	double ***dx=new double**[_z];
	double ***ds=new double**[_z];
	for(int iz=0; iz<_z; iz++){
		dx[iz]=new double*[_y];
		ds[iz]=new double*[_y];
		for(int iy=0; iy<_y; iy++){
			dx[iz][iy]=new double[_x];
			ds[iz][iy]=new double[_x];
		}
	}
...
void BSplineBiasField::Write(char *name)
{
	...
	unsigned int trans_type = MIRTKBIASFIELD_BSPLINE;
