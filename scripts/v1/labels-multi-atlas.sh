...
# hwm = wm_a * hwm_t/wm_t
run mirtk calculate $sdir/tissue-posteriors/hwm/$subj.nii.gz -div $sdir/tissue-posteriors/$subj-wmall.nii.gz -mul $sdir/labels/wm/$subj.nii.gz -out $sdir/labels/hwm/$subj.nii.gz 
# lwm = wm_a * lwm_t/wm_t
run mirtk calculate $sdir/tissue-posteriors/lwm/$subj.nii.gz -div $sdir/tissue-posteriors/$subj-wmall.nii.gz -mul $sdir/labels/wm/$subj.nii.gz -out $sdir/labels/lwm/$subj.nii.gz 
# wm  = wm_a - lwm - hwm
...
run mirtk calculate $sdir/MADs/$subj-grad.nii.gz -div $sdir/MADs/$subj-cur.nii.gz -div 1.4826 -sq -mul 0.5 -add 1 -log -out $sdir/MADs/$subj-cur.nii.gz 
run mirtk calculate N4/$subj.nii.gz -div N4/$subj.nii.gz -mul $sdir/MADs/$subj-cur.nii.gz  -add 1 -out $sdir/MADs/$subj-cur.nii.gz 
run mirtk calculate $sdir/MADs/$subj-cur.nii.gz -mul 0 -add 1 -div $sdir/MADs/$subj-cur.nii.gz -out $sdir/MADs/$subj.nii.gz 
