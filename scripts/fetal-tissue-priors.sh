...
if [ $needtseg -eq 1 -a ! -f $sdir/tissue-initial-segmentations/$subj.nii.gz ];then
    ...
    structures="csf gm wm outlier ventricles cerebstem dgm germinal-matrix"
    ...
    run mirtk transform-image $DRAWEMDIR/atlases/$atlasname/atlas-8/structure$strnum/$age.nii.gz $strems -dofin $dof -target N4/$subj.nii.gz -interp Linear
    emsstructures="$emsstructures $strems"
    done

    mkdir -p logs
    run mirtk draw-em N4/$subj.nii.gz $strnum $emsstructures $sdir/tissue-initial-segmentations/$subj.nii.gz -padding 0 -mrf $DRAWEMDIR/parameters/conn_tissues_ven_cstem_dgm_gmat.mrf -tissues 1 3 1 0 1 1 1 2 -hui -relaxtimes 2 $emsposts  1>logs/$subj-tissue-em 2>logs/$subj-tissue-em-err
